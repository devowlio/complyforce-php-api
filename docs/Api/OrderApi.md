...
Stops a scan order that has not finished yet.  Authenticate with a vendor API key (`x-api-key`) or a vendor dashboard Bearer token. The order status moves to **`canceling`** right away while remaining page scans are stopped, then to **`canceled`** once cleanup is done.  Returns `204` when the cancel is accepted, including when the order is already `canceled` (idempotent). Returns `422` when the order cannot be canceled because it is already **`canceling`**, or has finished (`completed`, `completedPartially`, or `error`).

### Example
```php
<?php
require_once(__DIR__ . '/vendor/autoload.php');
// Configure API key authorization: vendorJwtOrApiKeyAuth
...
[vendorJwtOrApiKeyAuth](../../README.md#vendorJwtOrApiKeyAuth)
...
Returns one order: current status, optional status history, and — when available — the report link and summary.  Authenticate with a vendor API key (`x-api-key`) or a vendor dashboard Bearer token. You can only read orders that belong to a vendor you have access to.
...
Lists scan orders for a vendor, with cursor pagination and optional filters (date range, billing, status, brand, channel).  Authenticate with a vendor API key (`x-api-key`) or a vendor dashboard Bearer token.
...
$vendorUuid = new \DevOwl\ComplyforceApiClient\Model\null(); //  | Vendor UUID. The authenticated API key or JWT user must have access to this vendor.
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **vendorUuid** | [****](../Model/.md)| Vendor UUID. The authenticated API key or JWT user must have access to this vendor. |
 ...
 **billingId** | [****](../Model/.md)| Filter by vendor billing ID. | [optional]
